...
// Everything that differs between a driver and a store lives here, so the
// component below stays role-agnostic.
const roleConfig = {
  driver: {
    endpoint: "driver",
    ...
    // Extra detail rows shown only for drivers
...
export default function PendingApproval({ role }: PendingApprovalProps) {
  ...
  const fetchProfile = async () => {
    try {
      setError(null);
      const res = await fetch(`${API_BASE}/${config.endpoint}/me`, {
        method: "GET",
        credentials: "include", // sends the Access_Token cookie
      });
      ...
      // Backend returns the profile under "driver" or "store" depending on role
      setProfile(data[config.endpoint]);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Something went wrong.");
    }
  };
