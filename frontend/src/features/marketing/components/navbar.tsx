    ...
    <header className="sticky top-0 z-50 border-b border-[#241710]/[0.06] bg-[#F8F2EA]/90 backdrop-blur-md">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <button onClick={() => scrollTo("top")} className="flex items-center gap-2">
          ...
          <span className="text-[1.15rem] font-semibold tracking-tight text-[#241710]" style={{ fontFamily: "Fraunces, serif" }}>
            QuickKart
          </span>
        </button>
        ...
        <ul className="hidden items-center gap-8 text-sm font-medium text-[#241710]/70 md:flex">
          {navLinks.map((l) => (
            <li key={l.label}>
              <button onClick={l.action} className="transition-colors hover:text-[#241710]">
                {l.label}
              </button>
            </li>
          ))}
        ...
        <div className="hidden items-center gap-5 md:flex">
          <Link to="/login" className="text-sm font-medium text-[#241710]/70 transition-colors hover:text-[#241710]">
            Log in
          </Link>
          <button
        ...
        {/* Mobile hamburger */}
        <button className="text-[#241710] md:hidden" onClick={() => setOpen((o) => !o)} aria-label="Toggle menu">
      ...
      {open && (
        ...
        >
          <ul className="flex flex-col gap-4 text-sm font-medium text-[#241710]/80">
            ...
            <li className="pt-1">
              <Link to="/login" className="block text-[#241710]/70" onClick={() => setOpen(false)}>
                Log in
              </Link>
            </li>
            <li>
              <button
                ...
                className="block w-full rounded-full bg-[#C2825A] px-5 py-2.5 text-center font-semibold text-[#241710]"
              >
                Get started
              </button>
            </li>
